...
import os
import json
from configuracion import ANCHO, ALTO, BASE_DIR, BLANCO, NEGRO, ROJO, VERDE, ORO, GRIS_CLARO, AMARILLO, CIAN, fmt_num
        ...
        if len(self.partidas) > 0:
            ...
            for i, partida in enumerate(self.partidas):
                ...
                txt_datos = self.fuente_peq.render(
                    f"Dinero: ${fmt_num(int(partida.get('dinero', 0)))} | Ano: {fmt_num(partida.get('ano', 0))} | Poblacion: {fmt_num(partida.get('poblacion', 0))} | Capitulo: {partida.get('capitulo', 1)}",
                    True, BLANCO
                )
