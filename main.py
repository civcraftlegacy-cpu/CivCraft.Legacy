...
from configuracion import fmt_num
...
class Juego:
    def __init__(self):
        ...
        self.mostrando_popup_nuevo_capitulo = False
        self.popup_recompensa_cofre = None  # None o {"nivel": 20/50/100}
        ...
        # --- ESTADO DE JUEGO Y ESCENA DE INVESTIGACION ---
    ...
    def dibujar_sub_stats(self, x, balance):
        ...
        if balance > 0:
            texto_pantalla = f"+{fmt_num(balance)}"
        else:
            texto_pantalla = f"-{fmt_num(abs(balance))}"
    ...
    def calcular_detalles_estado(self, tipo):
        ...
        if tipo == "felicidad":
            ...
            impacto_edificios_felic = round(sum(ed[6] * self.edificios_construidos.get(ed[0], 0) for ed in edificios_no_casa), 1)
            detalles.append(("Edificaciones", impacto_edificios_felic))
            total += impacto_edificios_felic
            ...
            edificios_no_casa = [ed for ed in EDIFICACIONES if ed[0] != "Casa"]
            impacto_edificios_salud = round(sum(ed[7] * self.edificios_construidos.get(ed[0], 0) for ed in edificios_no_casa), 1)
            detalles.append(("Edificaciones", impacto_edificios_salud))
            total += impacto_edificios_salud
    ...
    def dibujar_hud(self):
        ...
        comida_viz = int(self.logica.recursos['comida'])
        agua_viz = int(self.logica.recursos['agua'])
        energia_viz = int(self.logica.recursos['electricidad'])
        
        stats = [
            ("dinero", f"${fmt_num(self.logica.dinero)}", ORO, bal_dinero),
            ("poblacion", f"{fmt_num(n_pob)}/{fmt_num(self.logica.capacidad_max_poblacion)}", BLANCO, None),
            ("comida", f"{fmt_num(comida_viz)}", (0, 255, 100), bal_comida),
            ("agua", f"{fmt_num(agua_viz)}", CIAN, bal_agua),
            ("energia", f"{fmt_num(energia_viz)}", AMARILLO, bal_elec),
            ("felicidad", f"{int(fel)}%", VIOLETA, None),
            ("salud", f"{int(sal)}%", ROJO, None),
            ("reloj", f"Ano: {fmt_num(self.logica.ano)}", BLANCO, None)
        ]
    ...
    def dibujar_tienda(self):
        ...
        def blit_c(val, k, col, y_pos):
            txt = self.fuente_p.render(fmt_num(val) if isinstance(val, (int, float)) else str(val), True, col)
        ...
        # Guardar para limitar scroll en el manejador de eventos
        self.total_items_tienda = indice_visible
    ...
    def dibujar_inventario(self):
        ...
        for i, ed in enumerate(edificios_a_mostrar):
            ...
            for j, val in enumerate(stats):
                ...
                signo = "+" if val > 0 else ("-" if val < 0 else "")
                txt_v = self.fuente_p.render(f"{signo}{fmt_num(abs(val))}" if val != 0 else "0", True, color_v)
    ...
    def dibujar_dialogo_dinero_insuficiente(self):
        ...
        txt_falta = self.fuente_p.render(f"Te faltan: ${fmt_num(self.dinero_faltante)}", True, AMARILLO)
    ...
    def dibujar_menu_venta(self):
        ...
        if self.edificio_a_vender_seleccionado:
            ...
            txt_precio_original = self.fuente_p.render(f"Costo Original: ${fmt_num(costo_original)}", True, BLANCO)
            self.pantalla.blit(txt_precio_original, (panel_x + 20, panel_y + 90))
            
            txt_precio_venta = self.fuente_p.render(f"Precio Venta (50%): ${fmt_num(precio_venta)}", True, VERDE_BRILLANTE)
            ...
            txt_total = self.fuente_g.render(f"Total: ${fmt_num(total_venta)}", True, VERDE)
            ...
            for ed in edificios_a_mostrar:
                ...
                if btn_rect.top < scroll_area.bottom and btn_rect.bottom > scroll_area.top:
                    ...
                    txt_ed = self.fuente_p.render(f"{ed[0]} ({cant_disponible}x) - ${fmt_num(ed[1]//2)} c/u", True, BLANCO)
    ...
    def dibujar_botones_circulares(self):
        ...
        hay_mision_reclamable = any(
            self.logica.verificar_mision(i) and
            f"cap{self.logica.capitulo_actual}_{i}" not in self.logica.misiones_completadas
            for i in range(10)
        )
        if hay_mision_reclamable:
            pulso = int(abs(pygame.time.get_ticks() % 1000 - 500) / 500 * 155) + 100
            color_borde_mis = (0, pulso, 0)
            pygame.draw.circle(self.pantalla, color_borde_mis, self.btn_misiones.center, 33, 4)
            pygame.draw.circle(self.pantalla, color_borde_mis, self.btn_misiones.center, 30, 2)
            lbl_mis = self.fuente_p.render("Misiones", True, (0, 230, 80))
        else:
    ...
    def dibujar_misiones_popup(self):
        ...
        titulo = self.fuente_g.render("MISIONES", True, ORO)
        self.pantalla.blit(titulo, (panel_x + ancho_panel // 2 - titulo.get_width() // 2, panel_y + 12))

        # Botón X cerrar (cerrar.png) arriba a la derecha
        tam_cerrar = 28
        x_cerrar = panel_x + ancho_panel - tam_cerrar - 10
        y_cerrar = panel_y + 8
        img_cerrar_mis = pygame.transform.scale(self.img_cerrar, (tam_cerrar, tam_cerrar))
        self.pantalla.blit(img_cerrar_mis, (x_cerrar, y_cerrar))
        self.rect_misiones_cerrar = pygame.Rect(x_cerrar, y_cerrar, tam_cerrar, tam_cerrar)
        # Guardar rect del panel para detectar click fuera
        self.rect_panel_misiones = pygame.Rect(panel_x, panel_y, ancho_panel, alto_panel)

        progreso = min(100, int((len(self.logica.misiones_completadas) / max(len(self.logica.misiones_capitulo), 1)) * 100))

        # Cofres ENCIMA de la barra (solo los que no han sido abiertos)
        barra = pygame.Rect(panel_x + 30, panel_y + 108, ancho_panel - 60, 14)
        niveles = [(20, self.img_cofre1), (50, self.img_cofre2), (100, self.img_cofre3)]
        for porcentaje, imagen in niveles:
            if porcentaje in self.logica.cofres_abiertos:
                continue  # Cofre ya reclamado, no se dibuja
            x_icono = barra.x + int(barra.width * porcentaje / 100) - imagen.get_width() // 2
            y_icono = barra.y - imagen.get_height() - 4
        ...
        # Textos de las 9 misiones según el capítulo activo
        if self.logica.capitulo_actual == 1:
            textos_misiones = [
                ("Tener \u2265 100.000$",   ""),
                ...
                ("Salud \u2265 95%",        ""),
                ("\u2265 200 habitantes",   ""),
                ("10.000 de cada",          "recurso"),
            ]
        elif self.logica.capitulo_actual == 2:
            textos_misiones = [
                ("Felic \u2265 90%",        "y Salud \u2265 90%"),
                ("15.000 de",               "cada recurso"),
                ("3 almacenes",             "(1 de cada tipo)"),
                ("\u2265 350 hab.",          ""),
        ...
        for idx in range(9):
            ...
            self.rects_misiones.append(rect)

            mision_id = f"cap{self.logica.capitulo_actual}_{idx}"
            completada = mision_id in self.logica.misiones_completadas
            requisito_ok = self.logica.verificar_mision(idx)

            if completada:
                color_fondo = (30, 55, 30)
                color_borde = (80, 200, 80)
            elif requisito_ok:
                color_fondo = (35, 60, 35)
                color_borde = (0, 230, 80)
            else:
                color_fondo = (40, 40, 55)
                color_borde = (100, 100, 140)

            pygame.draw.rect(self.pantalla, color_fondo, rect, border_radius=8)
            pygame.draw.rect(self.pantalla, color_borde, rect, 2, border_radius=8)

            if completada:
                surf_done = self.fuente_m.render("COMPLETADO", True, (80, 220, 80))
                self.pantalla.blit(surf_done, (rect.centerx - surf_done.get_width() // 2, rect.centery - surf_done.get_height() // 2))
            else:
                linea1, linea2 = textos_misiones[idx]
                color_texto = (0, 230, 80) if requisito_ok else BLANCO
                surf1 = self.fuente_p.render(linea1, True, color_texto)
                if linea2:
                    surf2 = self.fuente_p.render(linea2, True, color_texto)
                    total_h = surf1.get_height() + surf2.get_height() + 3
                    y1 = rect.centery - total_h // 2
                    y2 = y1 + surf1.get_height() + 3
                    self.pantalla.blit(surf1, (rect.centerx - surf1.get_width() // 2, y1))
                    self.pantalla.blit(surf2, (rect.centerx - surf2.get_width() // 2, y2))
                else:
                    self.pantalla.blit(surf1, (rect.centerx - surf1.get_width() // 2, rect.centery - surf1.get_height() // 2))

        # Misión final (caja ancha)
        rect_especial = pygame.Rect(panel_x + 30, misiones_y + 3 * (casilla_alto + 10) + 10, ancho_panel - 60, casilla_alto)
        self.rect_especial_mision = rect_especial

        mision_esp_id = f"cap{self.logica.capitulo_actual}_9"
        esp_completada = mision_esp_id in self.logica.misiones_completadas
        esp_ok = self.logica.verificar_mision(9)

        if esp_completada:
            color_fondo_esp = (30, 55, 30)
            color_borde_esp = (80, 200, 80)
        elif esp_ok:
            color_fondo_esp = (35, 60, 35)
            color_borde_esp = (0, 230, 80)
        else:
            color_fondo_esp = (45, 45, 70)
            color_borde_esp = (200, 180, 100)

        pygame.draw.rect(self.pantalla, color_fondo_esp, rect_especial, border_radius=10)
        pygame.draw.rect(self.pantalla, color_borde_esp, rect_especial, 2, border_radius=10)

        if esp_completada:
            surf_done_esp = self.fuente_g.render("COMPLETADO", True, (80, 220, 80))
            self.pantalla.blit(surf_done_esp, (rect_especial.centerx - surf_done_esp.get_width() // 2, rect_especial.centery - surf_done_esp.get_height() // 2))
        else:
            color_esp = (0, 230, 80) if esp_ok else ORO
            color_esp2 = (0, 230, 80) if esp_ok else BLANCO
            if self.logica.capitulo_actual == 1:
                lin1_esp = self.fuente_p.render("+250 hab  \u00b7  \u226580% felic  \u00b7  \u226580% salud", True, color_esp)
                lin2_esp = self.fuente_p.render("30.000 de cualquier recurso", True, color_esp2)
            elif self.logica.capitulo_actual == 2:
                lin1_esp = self.fuente_p.render("200.000$  \u00b7  50.000 de cualquier recurso", True, color_esp)
                lin2_esp = self.fuente_p.render("", True, color_esp2)
            elif self.logica.capitulo_actual == 3:
                lin1_esp = self.fuente_p.render("500.000$  \u00b7  100.000 de cualquier recurso", True, color_esp)
                lin2_esp = self.fuente_p.render("", True, color_esp2)
            elif self.logica.capitulo_actual == 4:
                lin1_esp = self.fuente_p.render("3.000.000$  \u00b7  300.000 de cualquier recurso", True, color_esp)
                lin2_esp = self.fuente_p.render("", True, color_esp2)
            else:  # capítulo 5
                lin1_esp = self.fuente_p.render("10.000.000$  \u00b7  500.000 de cada recurso", True, color_esp)
                lin2_esp = self.fuente_p.render("", True, color_esp2)
            total_h_esp = lin1_esp.get_height() + lin2_esp.get_height() + 3
            y1_esp = rect_especial.centery - total_h_esp // 2
            y2_esp = y1_esp + lin1_esp.get_height() + 3
            self.pantalla.blit(lin1_esp, (rect_especial.centerx - lin1_esp.get_width() // 2, y1_esp))
            self.pantalla.blit(lin2_esp, (rect_especial.centerx - lin2_esp.get_width() // 2, y2_esp))

    # -----------------------------------------------------------------------
    # SISTEMA DE COFRES / RECOMPENSAS DE MISIONES
    # -----------------------------------------------------------------------
    def verificar_cofres(self):
        """Comprueba si se ha cruzado algún umbral de cofre y entrega la recompensa."""
        total = max(len(self.logica.misiones_capitulo), 1)
        completadas = len(self.logica.misiones_completadas)
        porcentaje = int(completadas / total * 100)
        cap = self.logica.capitulo_actual
        mult = 2 ** (cap - 1)  # x1, x2, x4, x8, x16

        if porcentaje >= 20 and 20 not in self.logica.cofres_abiertos:
            self.logica.cofres_abiertos.add(20)
            self.logica.dinero += 5000 * mult
            self.reproducir_sonido("dinero")
            self.popup_recompensa_cofre = {"nivel": 20}

        elif porcentaje >= 50 and 50 not in self.logica.cofres_abiertos:
            self.logica.cofres_abiertos.add(50)
            self.logica.dinero += 10000 * mult
            self.reproducir_sonido("dinero")
            self.popup_recompensa_cofre = {"nivel": 50}
            self.mostrando_selector_recurso_cofre = True

        elif porcentaje >= 100 and 100 not in self.logica.cofres_abiertos:
            self.logica.cofres_abiertos.add(100)
            self.logica.dinero += 50000 * mult
            self.reproducir_sonido("dinero")
            self.popup_recompensa_cofre = {"nivel": 100}
            self.mostrando_felicidades_capitulo1 = True

    def dibujar_popup_nuevo_capitulo(self):
        """Popup de felicitación al avanzar de capítulo."""
        cap = self.logica.capitulo_avanzado_a
        s = pygame.Surface((self.ancho, self.alto), pygame.SRCALPHA)
        s.fill((0, 0, 0, 180))
        self.pantalla.blit(s, (0, 0))

        cuadro = pygame.Rect(self.ancho // 2 - 300, self.alto // 2 - 140, 600, 280)
        pygame.draw.rect(self.pantalla, (15, 40, 15), cuadro, border_radius=18)
        pygame.draw.rect(self.pantalla, ORO, cuadro, 4, border_radius=18)

        estrella_txt = self.fuente_g.render("★  ¡FELICIDADES!  ★", True, ORO)
        self.pantalla.blit(estrella_txt, (cuadro.centerx - estrella_txt.get_width() // 2, cuadro.y + 18))

        lin1 = self.fuente_m.render("Has avanzado al", True, BLANCO)
        lin2 = self.fuente_g.render(f"CAPÍTULO {cap}", True, (100, 255, 100))
        self.pantalla.blit(lin1, (cuadro.centerx - lin1.get_width() // 2, cuadro.y + 72))
        self.pantalla.blit(lin2, (cuadro.centerx - lin2.get_width() // 2, cuadro.y + 102))

        desc = self.fuente_p.render(f"Años {(cap - 1) * 100 + 1} – {cap * 100}. ¡Nuevas misiones disponibles!", True, (200, 200, 255))
        self.pantalla.blit(desc, (cuadro.centerx - desc.get_width() // 2, cuadro.y + 158))

        self.btn_aceptar_nuevo_cap = pygame.Rect(cuadro.centerx - 90, cuadro.y + 200, 180, 50)
        pygame.draw.rect(self.pantalla, (0, 140, 0), self.btn_aceptar_nuevo_cap, border_radius=10)
        pygame.draw.rect(self.pantalla, ORO, self.btn_aceptar_nuevo_cap, 2, border_radius=10)
        txt_btn = self.fuente_m.render("¡ADELANTE!", True, BLANCO)
        self.pantalla.blit(txt_btn, (self.btn_aceptar_nuevo_cap.centerx - txt_btn.get_width() // 2,
                                     self.btn_aceptar_nuevo_cap.centery - txt_btn.get_height() // 2))

    def dibujar_popup_bloqueo_capitulo(self):
        """Popup que sale al intentar pasar del fin de capítulo sin completar misiones."""
        cap = self.logica.capitulo_actual
        ano_limite = cap * 100
        s = pygame.Surface((self.ancho, self.alto), pygame.SRCALPHA)
        s.fill((0, 0, 0, 180))
        ...
        cuadro = pygame.Rect(self.ancho // 2 - 310, self.alto // 2 - 130, 620, 260)
        pygame.draw.rect(self.pantalla, (30, 20, 20), cuadro, border_radius=15)
        pygame.draw.rect(self.pantalla, ROJO, cuadro, 4, border_radius=15)

        titulo = self.fuente_g.render("¡CAPÍTULO BLOQUEADO!", True, ROJO)
        self.pantalla.blit(titulo, (cuadro.centerx - titulo.get_width() // 2, cuadro.y + 18))

        lineas = [
            "No has cumplido todas las misiones.",
            f"Tienes que reiniciar el Capítulo {cap} para continuar.",
        ]
        y = cuadro.y + 70
        for linea in lineas:
            txt = self.fuente_m.render(linea, True, BLANCO)
            self.pantalla.blit(txt, (cuadro.centerx - txt.get_width() // 2, y))
            y += 32

        self.btn_reiniciar_bloqueo = pygame.Rect(cuadro.centerx - 100, cuadro.y + 185, 200, 50)
        pygame.draw.rect(self.pantalla, ROJO, self.btn_reiniciar_bloqueo, border_radius=10)
        pygame.draw.rect(self.pantalla, ORO, self.btn_reiniciar_bloqueo, 2, border_radius=10)
        txt_btn = self.fuente_m.render("REINICIAR CAPÍTULO", True, BLANCO)
        self.pantalla.blit(txt_btn, (self.btn_reiniciar_bloqueo.centerx - txt_btn.get_width() // 2,
                                     self.btn_reiniciar_bloqueo.centery - txt_btn.get_height() // 2))

    def dibujar_popup_cofre_recompensa(self):
        """Popup que aparece al abrir un cofre de misiones."""
        if not self.popup_recompensa_cofre:
            return
        nivel = self.popup_recompensa_cofre["nivel"]

        s = pygame.Surface((self.ancho, self.alto), pygame.SRCALPHA)
        s.fill((0, 0, 0, 180))
        self.pantalla.blit(s, (0, 0))

        if nivel == 50 and self.mostrando_selector_recurso_cofre:
            # Panel más alto para los 3 botones de recurso
            cuadro = pygame.Rect(self.ancho // 2 - 270, self.alto // 2 - 160, 540, 320)
        elif nivel == 100:
            cuadro = pygame.Rect(self.ancho // 2 - 300, self.alto // 2 - 160, 600, 320)
        else:
            cuadro = pygame.Rect(self.ancho // 2 - 240, self.alto // 2 - 110, 480, 220)

        pygame.draw.rect(self.pantalla, (20, 30, 20), cuadro, border_radius=15)
        pygame.draw.rect(self.pantalla, ORO, cuadro, 4, border_radius=15)

        # Icono cofre centrado arriba
        cap = self.logica.capitulo_actual
        mult = 2 ** (cap - 1)
        if nivel == 20:
            img_cofre = self.img_cofre1
            dinero_txt = f"¡Has recibido {fmt_num(5000 * mult)}$!"
        elif nivel == 50:
            img_cofre = self.img_cofre2
            dinero_txt = f"¡Has recibido {fmt_num(10000 * mult)}$!"
        else:
            img_cofre = self.img_cofre3
            dinero_txt = f"¡Has recibido {fmt_num(50000 * mult)}$!"

        cofre_grande = pygame.transform.scale(img_cofre, (50, 50))
        self.pantalla.blit(cofre_grande, (cuadro.centerx - 25, cuadro.y + 12))

        txt_dinero = self.fuente_g.render(dinero_txt, True, ORO)
        self.pantalla.blit(txt_dinero, (cuadro.centerx - txt_dinero.get_width() // 2, cuadro.y + 70))

        if nivel == 100:
            cap = self.logica.capitulo_actual  # el capítulo aún no ha avanzado
            ano_limite = cap * 100
            txt_feliz1 = self.fuente_m.render(f"¡FELICIDADES! ¡HAS COMPLETADO", True, (100, 255, 100))
            txt_feliz2 = self.fuente_m.render(f"EL CAPÍTULO {cap}!", True, (100, 255, 100))
            self.pantalla.blit(txt_feliz1, (cuadro.centerx - txt_feliz1.get_width() // 2, cuadro.y + 110))
            self.pantalla.blit(txt_feliz2, (cuadro.centerx - txt_feliz2.get_width() // 2, cuadro.y + 140))
            txt_desc = self.fuente_p.render(f"Ya puedes avanzar más allá del año {ano_limite}.", True, BLANCO)
            self.pantalla.blit(txt_desc, (cuadro.centerx - txt_desc.get_width() // 2, cuadro.y + 175))

            self.btn_aceptar_cofre = pygame.Rect(cuadro.centerx - 80, cuadro.y + 250, 160, 48)
            pygame.draw.rect(self.pantalla, (0, 150, 0), self.btn_aceptar_cofre, border_radius=10)
            pygame.draw.rect(self.pantalla, ORO, self.btn_aceptar_cofre, 2, border_radius=10)
            txt_ok = self.fuente_m.render("¡GENIAL!", True, BLANCO)
            self.pantalla.blit(txt_ok, (self.btn_aceptar_cofre.centerx - txt_ok.get_width() // 2,
                                        self.btn_aceptar_cofre.centery - txt_ok.get_height() // 2))

        elif nivel == 50 and self.mostrando_selector_recurso_cofre:
            cantidad_rec = 10000 * (2 ** (self.logica.capitulo_actual - 1))
            txt_elige = self.fuente_m.render(f"Elige un recurso para recibir {fmt_num(cantidad_rec)} unidades:", True, BLANCO)
            self.pantalla.blit(txt_elige, (cuadro.centerx - txt_elige.get_width() // 2, cuadro.y + 110))

            nombres_rec = [("comida", "COMIDA"), ("agua", "AGUA"), ("electricidad", "ELECTRICIDAD")]
            colores_rec = [(80, 200, 80), (80, 150, 255), (255, 220, 50)]
            ancho_btn = 140
            separacion = 20
            total_ancho = ancho_btn * 3 + separacion * 2
            x_ini = cuadro.centerx - total_ancho // 2
            self.btns_recurso_cofre = []
            for idx_r, ((clave, etiqueta), color) in enumerate(zip(nombres_rec, colores_rec)):
                btn_r = pygame.Rect(x_ini + idx_r * (ancho_btn + separacion), cuadro.y + 155, ancho_btn, 50)
                self.btns_recurso_cofre.append((btn_r, clave))
                pygame.draw.rect(self.pantalla, color, btn_r, border_radius=10)
                pygame.draw.rect(self.pantalla, ORO, btn_r, 2, border_radius=10)
                txt_r = self.fuente_m.render(etiqueta, True, (20, 20, 20))
                self.pantalla.blit(txt_r, (btn_r.centerx - txt_r.get_width() // 2,
                                           btn_r.centery - txt_r.get_height() // 2))

            self.btn_cancelar_cofre50 = None  # No hay cancelar – hay que elegir

        else:
            # nivel == 20 o nivel == 50 ya elegido
            self.btn_aceptar_cofre = pygame.Rect(cuadro.centerx - 70, cuadro.y + 155, 140, 48)
            pygame.draw.rect(self.pantalla, (0, 150, 0), self.btn_aceptar_cofre, border_radius=10)
            pygame.draw.rect(self.pantalla, ORO, self.btn_aceptar_cofre, 2, border_radius=10)
            txt_ok = self.fuente_m.render("¡GENIAL!", True, BLANCO)
            self.pantalla.blit(txt_ok, (self.btn_aceptar_cofre.centerx - txt_ok.get_width() // 2,
    ...
    async def ejecutar(self):
        while self.corriendo:
            ...
            # ================================================================
            if self.estado_actual == 'INVESTIGACION':
                # Mostrar la ciudad congelada como fondo (o negro si aún no hay captura)
                if self.ciudad_surface:
                    self.pantalla.blit(self.ciudad_surface, (0, 0))
                else:
                    self.pantalla.fill(NEGRO)
                self.menu_inv.dibujar(self.pantalla, self.logica.capitulo_actual)
            ...
            # ================================================================
            # ESTADO: CIUDAD (lógica principal)
            # ================================================================
            self.pantalla.fill(NEGRO)
            ...
            # --- POPUPS DE COFRES Y BLOQUEO DE CAPÍTULO ---
            if self.mostrando_popup_bloqueo_capitulo:
                self.dibujar_popup_bloqueo_capitulo()

            if self.mostrando_popup_nuevo_capitulo:
                self.dibujar_popup_nuevo_capitulo()
                ...
                # --- EVENTO DE TECLADO ---
                    ...
                    if self.campo_nombre_activo and self.dialogo_guardar_abierto:
                        if ev.key == pygame.K_BACKSPACE:
                            ...
                            resultado_guardado = self.logica.guardar_partida_con_nombre(self.input_nombre_partida if self.input_nombre_partida else f"Año {self.logica.ano}")
                    ...
                    if self.menu_venta_abierto:
                        ...
                        if not self.edificio_a_vender_seleccionado:
                            ...
                            if hasattr(self, 'btn_venta_si') and self.btn_venta_si.collidepoint(pos):
                                # Ejecutar venta
                                nombre_ed = self.edificio_a_vender_seleccionado[0]
                                disponibles = self.edificios_construidos.get(nombre_ed, 0)
                                cantidad_real = min(self.cantidad_a_vender, disponibles)
                                resultado = self.logica.vender_edificios(nombre_ed, cantidad_real)
                                if resultado:
                                    self.edificios_construidos[nombre_ed] -= cantidad_real
                                    self.logica.noticias.append({"txt": f"Vendiste {cantidad_real}x {nombre_ed}", "tipo": "AVISO"})
                    ...
                    if self.menu_compra_abierto:
                        if self.btn_cerrar.collidepoint(pos):
                            ...
                            min_scroll = min(0, 480 - getattr(self, 'total_items_tienda', 0) * 55)
                            self.scroll_y = max(self.scroll_y - 30, min_scroll)
                        else:
                    ...
                    # 5.8b Popup bloqueo de capítulo (misiones incompletas al llegar año 100)
                    if self.mostrando_popup_bloqueo_capitulo:
                    ...
                    # 5.8d Popup nuevo capítulo (¡Felicidades!)
                    if self.mostrando_popup_nuevo_capitulo:
                        if hasattr(self, 'btn_aceptar_nuevo_cap') and self.btn_aceptar_nuevo_cap.collidepoint(pos):
                            self.mostrando_popup_nuevo_capitulo = False
                            self.reproducir_sonido("cerrar")
                        continue
                    if self.popup_recompensa_cofre:
                        nivel = self.popup_recompensa_cofre["nivel"]
                        if nivel == 50 and self.mostrando_selector_recurso_cofre:
                            # Esperar click en uno de los 3 botones de recurso
                            cantidad_rec = 10000 * (2 ** (self.logica.capitulo_actual - 1))
                            for btn_r, clave_rec in getattr(self, 'btns_recurso_cofre', []):
                            ...
                            if hasattr(self, 'btn_aceptar_cofre') and self.btn_aceptar_cofre.collidepoint(pos):
                                es_nivel100 = nivel == 100
                                self.popup_recompensa_cofre = None
                                self.mostrando_felicidades_capitulo1 = False
                                self.reproducir_sonido("cerrar")
                        continue

                    # 5.9 Popup de misiones abierto
                    if self.mostrando_misiones:
                        # Cerrar con X
                        if hasattr(self, 'rect_misiones_cerrar') and self.rect_misiones_cerrar.collidepoint(pos):
                            self.mostrando_misiones = False
                            self.reproducir_sonido("cerrar")
                            continue
                        # Cerrar al hacer click fuera del panel
                        if hasattr(self, 'rect_panel_misiones') and not self.rect_panel_misiones.collidepoint(pos):
                            self.mostrando_misiones = False
                            self.reproducir_sonido("cerrar")
                            continue
                        reclamada = False
                        for i, rect in enumerate(getattr(self, 'rects_misiones', [])):
                            if rect.collidepoint(pos):
                                mision_id = f"cap{self.logica.capitulo_actual}_{i}"
                                if (self.logica.verificar_mision(i) and
                                        mision_id not in self.logica.misiones_completadas):
                                    self.logica.misiones_completadas.add(mision_id)
                                    self.verificar_cofres()
                                    self.reproducir_sonido("cerrar")
                                reclamada = True
                                break
                        if not reclamada and hasattr(self, 'rect_especial_mision') and self.rect_especial_mision and self.rect_especial_mision.collidepoint(pos):
                            mision_id = f"cap{self.logica.capitulo_actual}_9"
                            if (self.logica.verificar_mision(9) and
                                    mision_id not in self.logica.misiones_completadas):
                                self.logica.misiones_completadas.add(mision_id)
                                self.verificar_cofres()
                                self.reproducir_sonido("cerrar")
                    ...
                    if self.btn_next.collidepoint(pos) and self.boton_next_habilitado:
                        ...
                        # Comprobar si el avance fue bloqueado por misiones incompletas
                        if self.logica.mostrar_popup_bloqueo_capitulo:
                            self.mostrando_popup_bloqueo_capitulo = True
                            self.logica.mostrar_popup_bloqueo_capitulo = False
                        # Comprobar si se avanzó de capítulo correctamente
                        if self.logica.mostrar_popup_nuevo_capitulo:
                            self.mostrando_popup_nuevo_capitulo = True
                            self.logica.mostrar_popup_nuevo_capitulo = False
                        self.boton_next_habilitado = True
                        ...
                        self.ciudad_surface = self.pantalla.copy()  # congela el frame actual
